...
vec3 CalcDirLight(DirLight light, vec3 normal,vec3 fragPos, vec3 viewDir){
    ...
    float spec = pow(max(dot(viewDir, reflectDir),0.0),64.0)*2.0;

    vec3 ambient = light.ambient * 2.0;
    vec3 diffuse = light.diffuse * diff * 2.0;
    vec3 specular = light.specular * spec * 2.0;
...
void main()
{
    ...
    //Directional Light
    vec3 result = CalcDirLight(dirLight,normal,FragPos,viewDir);
//    vec3 result;
